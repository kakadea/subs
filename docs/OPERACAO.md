...
Na seção **Fontes** do painel administrativo há duas partes. **Fontes para instalar** aceita arquivos reais de tipografia `.ttf`, `.otf` e `.ttc`, inclusive em lote, para que o visitante possa baixar e instalar no Windows. **Links relacionados** aceita URLs HTTPS opcionais, como páginas de download dos episódios ou outras referências. Os arquivos tipográficos são validados pela assinatura binária, armazenados fora do `public_html` com nome derivado de SHA-256 e removidos definitivamente junto com o registro quando excluídos. Tanto fontes quanto links só aparecem publicamente quando o projeto estiver compartilhado; projetos privados permanecem restritos ao admin.

O catálogo público lista projetos, não arquivos soltos. A página pública do projeto possui as abas **Legendas** e **Fontes**. A aba Fontes mostra os arquivos de tipografia para baixar e, abaixo, os links externos relacionados. A fonte padrão dos metadados é a Tenrai, uma API pública com esquema compatível com os campos necessários da Jikan/MAL; a URL do provedor fica configurável para permitir substituição futura.
